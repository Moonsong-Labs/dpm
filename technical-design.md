...
- A warm machine with a populated cache can resolve and compile offline.
...
- Otherwise `dpm` reuses or creates a working clone of that repository, one clone per host, org, and repo, kept next to the cache. It fetches the requested revision, checks out the commit that revision currently names, and copies the DAR file out of the worktree.
